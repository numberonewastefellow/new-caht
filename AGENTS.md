...
- You should assume that all VertualAI services are running. To verify, you can check the `backend/log` directory to
...
VertualAI uses Celery for asynchronous task processing with multiple specialized workers:
...
VertualAI supports two deployment modes for background workers, controlled by the `USE_LIGHTWEIGHT_BACKGROUND_WORKER` environment variable:
...
If you need help with this step, reach out to `raunak@vertualai.app`.
...
There are 4 main types of tests within VertualAI:

### Unit Tests

These should not assume any VertualAI/external services are available to be called.
...
These tests assume that all external dependencies of VertualAI are available and callable (e.g. Postgres, Redis,
MinIO/S3, Vespa are running + OpenAI can be called + any request to the internet is fine + etc.).

However, the actual VertualAI containers are not running and with these tests we call the function to test directly.
...
Standard integration tests. Every test in `backend/tests/integration` runs against a real VertualAI deployment. We cannot
...
These tests are an even more complete version of the Integration Tests mentioned above. Has all services of VertualAI
...
to logs via the `backend/log/<service_name>_debug.log` file. All VertualAI services (api_server, web_server, celery_X)
