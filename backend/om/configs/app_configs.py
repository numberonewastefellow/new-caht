...
from om.file_processing.enums import HtmlBasedConnectorTransformLinksStrategy
from om.prompts.image_analysis import DEFAULT_IMAGE_SUMMARIZATION_SYSTEM_PROMPT
from om.prompts.image_analysis import DEFAULT_IMAGE_SUMMARIZATION_USER_PROMPT
...
# codepath was actually configuring this, so we'll just use the default value,
# but also have it be configurable by env var.
RECENCY_BIAS_MULTIPLIER = float(os.environ.get("RECENCY_BIAS_MULTIPLIER") or 1.0)

# Should match the rerank-count value that was historically configured in the
# Vespa schema, which no longer exists.
...
# when the capture is called. These defaults prevent Posthog issues from breaking the app
