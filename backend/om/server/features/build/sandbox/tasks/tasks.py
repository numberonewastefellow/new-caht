...
from om.server.features.build.configs import SANDBOX_IDLE_TIMEOUT_SECONDS
from om.server.features.build.configs import SandboxBackend
from om.server.features.build.configs import USER_LIBRARY_SOURCE_DIR
from om.server.features.build.db.build_session import clear_nextjs_ports_for_user
...
from om.server.features.build.sandbox.kubernetes.kubernetes_sandbox_manager import (
...
def cleanup_idle_sandboxes_task(self: Task, *, tenant_id: str) -> None:  # noqa: ARG001
    ...
    try:
        ...
        from om.db.enums import SandboxStatus
        from om.server.features.build.db.sandbox import create_snapshot__no_commit
        from om.server.features.build.db.sandbox import get_idle_sandboxes
...
def _get_disabled_user_library_paths(db_session: "Session", user_id: str) -> list[str]:
    ...
    from om.db.document import get_documents_by_source
...
#     try:
#         from om.server.features.build.db.sandbox import delete_old_snapshots
