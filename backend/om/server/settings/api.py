...
def get_settings_notifications(user: User, db_session: Session) -> list[Notification]:
    """Get notifications for the settings page (reindex notifications for admins).

    WS-A: the trial/product-gating notification (``TRIAL_ENDS_TWO_DAYS``) was
    removed with billing — nothing emits it anymore, so the settings page only
    surfaces reindex notifications now.
    """
