...
from om.background.celery.tasks.document_index.document_sync import DOCUMENT_SYNC_FENCE_KEY
from om.background.celery.tasks.document_index.document_sync import get_document_sync_payload
from om.background.celery.tasks.document_index.document_sync import get_document_sync_remaining
from om.background.celery.tasks.document_index.document_sync import reset_document_sync
from om.background.celery.tasks.document_index.document_sync import (
    try_generate_stale_document_sync_tasks,
)
from om.configs.app_configs import JOB_TIMEOUT
from om.configs.app_configs import DOC_INDEX_SYNC_MAX_TASKS
...
from om.db.document_set import fetch_document_sets
from om.db.document_set import fetch_document_sets_for_document
from om.db.document_set import get_document_set_by_id
from om.db.document_set import mark_document_set_as_synced
...
@shared_task(
    name=OmCeleryTask.CHECK_FOR_DOC_INDEX_SYNC_TASK,
    ignore_result=True,
...
def check_for_document_index_sync_task(self: Task, *, tenant_id: str) -> bool | None:
    ...
    lock_beat: RedisLock = r.lock(
        OmRedisLocks.CHECK_DOC_INDEX_SYNC_BEAT_LOCK,
        timeout=CELERY_DOC_INDEX_SYNC_BEAT_LOCK_TIMEOUT,
    )
    ...
    try:
        ...
        with get_session_with_current_tenant() as db_session:
            try_generate_stale_document_sync_tasks(
                self.app, DOC_INDEX_SYNC_MAX_TASKS, db_session, r, lock_beat, tenant_id
            )
        ...
        if lock_beat.owned():
            ...
            task_logger.error(
                "check_for_document_index_sync_task - Lock not owned on completion: "
                f"tenant={tenant_id}"
            )
...
@shared_task(
    name=OmCeleryTask.DOC_INDEX_METADATA_SYNC_TASK,
    bind=True,
