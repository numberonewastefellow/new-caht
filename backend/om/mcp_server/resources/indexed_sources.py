...
from om.mcp_server.api import mcp_server
from om.mcp_server.utils import get_indexed_sources
from om.mcp_server.utils import require_access_token
...
@mcp_server.resource(
    ...
    description=(
        "Enumerate the user's document sources that are currently indexed in VertualAI."
...
async def indexed_sources_resource() -> dict[str, Any]:
    ...
    logger.info(
        "VertualAI MCP Server: indexed_sources resource returning %s entries",
        len(sources),
    )
