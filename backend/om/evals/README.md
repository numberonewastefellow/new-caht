# VertualAI Evaluations

This directory contains the evaluation framework for testing and measuring the performance of VertualAI's chat and retrieval systems.

## Overview

The evaluation system uses [Braintrust](https://www.braintrust.dev/) to run automated evaluations against test datasets. It measures the quality of responses generated by VertualAI's chat system and can be used to track performance improvements over time.
...
```bash
onyx$ python -m dotenv -f .vscode/.env run -- python backend/om/evals/eval_cli.py --local-dataset-path backend/om/evals/data/eval.json --search-permissions-email richard@vertualai.app
```
Save the env var OM_EVAL_API_KEY in your .env file so you don't have to specify it every time for triggering remote runs.
