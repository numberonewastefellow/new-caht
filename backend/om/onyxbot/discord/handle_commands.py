...
from om.onyxbot.discord.constants import REGISTER_COMMAND
from om.onyxbot.discord.constants import SYNC_CHANNELS_COMMAND
from om.onyxbot.discord.exceptions import RegistrationError
from om.onyxbot.discord.exceptions import SyncChannelsError
from om.server.manage.discord_bot.utils import parse_discord_registration_key
...
async def handle_registration_command(
    ...
    try:
        ...
        await message.reply(
            ...
            "This server is now connected to VertualAI. "
            "I'll respond to messages based on your server and channel settings set in VertualAI."
        )
...
async def _register_guild(
    ...
    if existing_tenant is not None:
        ...
        raise RegistrationError(
            ...
            "VertualAI Bot can only connect one Discord server to one VertualAI workspace."
        )
            ...
            with get_session_with_tenant(tenant_id=tenant_id) as db:
                ...
                if not config:
                    raise RegistrationError(
                        ...
                        "Please generate a new one from the VertualAI admin panel."
                    )
                ...
                if config.guild_id is not None:
                    raise RegistrationError(
                        ...
                        "Please generate a new key from the VertualAI admin panel."
                    )
    ...
    try:
        ...
        def _get_guild_config_id() -> int | None:
            with get_session_with_tenant(tenant_id=tenant_id) as db:
                if not message.guild:
                    raise SyncChannelsError(
                        "Server not found. This shouldn't happen. Please contact VertualAI support."
                    )
        ...
        if not guild_config_id:
            raise SyncChannelsError(
                "Server config not found. This shouldn't happen. Please contact VertualAI support."
            )
        ...
        await message.reply(
            ...
            "New channels are disabled by default. Enable them in the VertualAI admin panel."
        )
