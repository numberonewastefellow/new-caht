...
from om.natural_language_processing.search_nlp_models import warm_up_bi_encoder
from om.onyxbot.slack.config import get_slack_channel_config_for_bot_and_channel
from om.onyxbot.slack.config import MAX_TENANTS_PER_POD
from om.onyxbot.slack.config import TENANT_ACQUISITION_INTERVAL
from om.onyxbot.slack.config import TENANT_HEARTBEAT_EXPIRATION
from om.onyxbot.slack.config import TENANT_HEARTBEAT_INTERVAL
from om.onyxbot.slack.config import TENANT_LOCK_EXPIRATION
from om.onyxbot.slack.constants import DISLIKE_BLOCK_ACTION_ID
from om.onyxbot.slack.constants import FEEDBACK_DOC_BUTTON_BLOCK_ACTION_ID
from om.onyxbot.slack.constants import FOLLOWUP_BUTTON_ACTION_ID
from om.onyxbot.slack.constants import FOLLOWUP_BUTTON_RESOLVED_ACTION_ID
from om.onyxbot.slack.constants import GENERATE_ANSWER_BUTTON_ACTION_ID
from om.onyxbot.slack.constants import IMMEDIATE_RESOLVED_BUTTON_ACTION_ID
from om.onyxbot.slack.constants import KEEP_TO_YOURSELF_ACTION_ID
from om.onyxbot.slack.constants import LIKE_BLOCK_ACTION_ID
from om.onyxbot.slack.constants import SHOW_EVERYONE_ACTION_ID
from om.onyxbot.slack.constants import VIEW_DOC_FEEDBACK_ID
from om.onyxbot.slack.handlers.handle_buttons import handle_doc_feedback_button
from om.onyxbot.slack.handlers.handle_buttons import handle_followup_button
from om.onyxbot.slack.handlers.handle_buttons import (
...
from om.onyxbot.slack.handlers.handle_buttons import handle_slack_feedback
from om.onyxbot.slack.handlers.handle_message import handle_message
from om.onyxbot.slack.handlers.handle_message import (
    remove_scheduled_feedback_reminder,
)
from om.onyxbot.slack.handlers.handle_message import schedule_feedback_reminder
...
from om.onyxbot.slack.utils import check_message_limit
from om.onyxbot.slack.utils import decompose_action_id
from om.onyxbot.slack.utils import get_channel_name_from_id
from om.onyxbot.slack.utils import get_channel_type_from_id
from om.onyxbot.slack.utils import get_onyx_bot_auth_ids
from om.onyxbot.slack.utils import read_slack_thread
from om.onyxbot.slack.utils import remove_onyx_bot_tag
from om.onyxbot.slack.utils import respond_in_thread_or_channel
from om.onyxbot.slack.utils import TenantSocketModeClient
...
def prefilter_requests(req: SocketModeRequest, client: TenantSocketModeClient) -> bool:
    ...
    if req.type == "events_api":
        ...
        if event_type == "message":
            ...
            # VertualAi Bot should never respond to itself
            if is_onyx_bot_msg:
                logger.info("Ignoring message from VertualAi Bot (self-message)")
                return False

            # DMs with the bot don't pick up the @VertualAi Bot so we have to keep the
            # caught events_api
        ...
        if is_bot_message:
            ...
            # If VertualAi Bot is not specifically tagged and the channel is not set to respond to bots, ignore the message
        ...
        # Can respond in thread if it's an "im" directly to VertualAi or @VertualAi Bot is tagged
        if (
            thread_ts
    ...
    if req.type == "slash_commands":
        ...
        if not channel:
            channel_specific_logger.error(
                "Received VertualAi Bot command without channel - skipping"
            )
            return False
        ...
        if not sender:
            channel_specific_logger.error(
                "Cannot respond to VertualAi Bot command without sender to respond to."
            )
            return False
...
def build_request_details(
    ...
    if req.type == "events_api":
        ...
        if tagged:
            logger.debug("User tagged VertualAi Bot")
