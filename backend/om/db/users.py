...
from om.configs.constants import NO_AUTH_PLACEHOLDER_USER_EMAIL
from om.db.api_key import DANSWER_API_KEY_DUMMY_EMAIL_DOMAIN
...
def validate_user_role_update(
    ...
    if current_role == UserRole.SLACK_USER:
        raise HTTPException(
            status_code=400,
            detail="To change a Slack User's role, they must first login to VertualAI via the web app.",
        )
    ...
    if current_role == UserRole.EXT_PERM_USER:
        ...
        raise HTTPException(
            status_code=400,
            detail="To change an External Permissioned User's role, they must first login to VertualAI via the web app.",
        )

    if current_role == UserRole.LIMITED:
        raise HTTPException(
            status_code=400,
            detail="To change a Limited User's role, they must first login to VertualAI via the web app.",
        )
    ...
    if requested_role == UserRole.SLACK_USER:
        ...
        raise HTTPException(
            ...
            detail=(
                ...
                "This role is automatically assigned to users who only use VertualAI via Slack."
            ),
        )
...
def delete_user_from_db(
    ...
    db_session.query(Agent).filter(Agent.user_id == user_to_delete.id).update(
        {Agent.user_id: None}
    )
