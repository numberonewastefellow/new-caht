...
class SelectionAnalysis:
    ...
    def do_request(self, query: str) -> dict:
        """Request the VertualAI API

        Args:
            query (str): A query

        Returns:
            dict: The VertualAI API response content
        """
        cookies = (
        ...
        try:
            ...
            if response.status_code != 200:
                color_output(
                    (
                        "something goes wrong while requesting the VertualAI API "
            ...
            color_output(
                f"Unable to request the VertualAI API for the query '{query}': {e}",
                model="critical",
            )
            sys.exit(1)
    ...
    def extract_content(self, contents: dict) -> dict:
        """Extract the content returns by the VertualAI API
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        default=None,
        help=(
            "Currently, to get this script working when the VertualAI Auth is "
    ...
    parser.add_argument(
        ...
        default=3000,
        help=(
            "The VertualAI Web (not the API) port. We use the UI to forward the requests to the API. "
            "It should be '3000' for local dev and '80' if VertualAI runs using docker compose."
        ),
    )
    parser.add_argument(
        "-q" "--queries",
