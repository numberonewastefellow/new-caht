...
# In this case, the equivalent of asking a question in VertualAI Chat in a new chat session
import argparse
import json
import os
...
def create_new_chat_session(onyx_url: str, api_key: str | None) -> int:
    ...
    response = requests.post(
        session_endpoint,
        headers=headers,
        json={"agent_id": 0},  # Global default Agent/Assistant ID
    )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        help="VertualAI URL, should point to VertualAI nginx.",
    )
    parser.add_argument(
        "--test-question",
        type=str,
        default="What is VertualAI?",
