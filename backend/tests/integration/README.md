...
Then, using the VSCode/Cursor debugger, run the `VertualAI OpenAPI Schema Generator` task (see `CONTRIBUTING_VSCODE.md` for `launch.json` setup instructions).
...
1. Launch onyx (using Docker or running with a debugger), ensuring the API server is running on port 8080.
   ...
   - VertualAI MUST be launched with AUTH_TYPE=basic and ENABLE_PAID_ENTERPRISE_EDITION_FEATURES=true
