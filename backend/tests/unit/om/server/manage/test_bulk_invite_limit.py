...
def test_trial_tenant_can_invite_within_limit(
    ...
    *_mocks: None,
    ...
    WS-A: billing was removed, so the invite path no longer syncs seats to a
    control plane (register_tenant_users) — it just adds the users to the tenant.
    """
    ...
    assert result == 3
    # New invitees are added to the tenant (no control-plane billing sync remains).
