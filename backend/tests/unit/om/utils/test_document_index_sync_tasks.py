...
from om.background.celery.tasks.document_index import tasks as doc_index_tasks
...
def _setup_common_patches(monkeypatch: Any, document_set: Any) -> dict[str, bool]:
    ...
    monkeypatch.setattr(doc_index_tasks, "RedisDocumentSet", _StubRedisDocumentSet)
    ...
    monkeypatch.setattr(doc_index_tasks, "delete_document_set", _delete)

    def _mark(document_set_id: Any, db_session: Any) -> None:  # noqa: ARG001
        calls["synced"] = True

    monkeypatch.setattr(doc_index_tasks, "mark_document_set_as_synced", _mark)
...
def test_monitor_preserves_federated_only_document_set(monkeypatch: Any) -> None:
    ...
    doc_index_tasks.monitor_document_set_taskset(
        tenant_id="tenant",
