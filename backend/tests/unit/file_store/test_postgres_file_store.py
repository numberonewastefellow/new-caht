...
class TestSaveFile:
    @patch(
        "om.file_store.postgres_file_store.get_session_with_current_tenant_if_none"
    )
    def test_save_bytes_content(
        ...
        with (
            patch(
                "om.file_store.postgres_file_store.upsert_filerecord"
            ) as mock_upsert_fr,
            patch(
                "om.file_store.postgres_file_store.upsert_file_content"
            ) as mock_upsert_fc,
        ):
    ...
    def test_save_string_io_content(
        ...
        with (
            patch("om.file_store.postgres_file_store.upsert_filerecord"),
            patch("om.file_store.postgres_file_store.upsert_file_content"),
        ):
...
class TestReadFile:
    ...
    def test_read_file_in_memory(
        ...
        with patch(
            "om.file_store.postgres_file_store.get_file_content_by_file_id",
...
class TestDeleteFile:
    ...
    def test_delete_removes_lobject_and_records(
        ...
        with (
            ...
            patch(
                "om.file_store.postgres_file_store.delete_file_content_by_file_id"
            ) as mock_del_fc,
            patch(
                "om.file_store.postgres_file_store.delete_filerecord_by_file_id"
            ) as mock_del_fr,
        ):
...
class TestChangeFileId:
    ...
    def test_reuses_same_lobject(
        ...
        with (
            patch(
                "om.file_store.postgres_file_store.get_filerecord_by_file_id",
                return_value=old_fr,
            ),
            patch(
                "om.file_store.postgres_file_store.upsert_filerecord"
            ...
            patch(
                "om.file_store.postgres_file_store.transfer_file_content_file_id"
            ) as mock_transfer,
            patch("om.file_store.postgres_file_store.delete_filerecord_by_file_id"),
        ):
...
class TestHasFile:
    ...
    def test_returns_true_when_present(
        ...
        with patch(
            "om.file_store.postgres_file_store.get_filerecord_by_file_id_optional",
            return_value=record,
        ):
