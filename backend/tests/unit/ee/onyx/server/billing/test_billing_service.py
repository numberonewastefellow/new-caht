...
class TestMakeBillingRequest:
    ...
    @pytest.mark.asyncio
    @patch("om.server.billing.service._get_headers")
    @patch("om.server.billing.service._get_base_url")
    async def test_makes_post_request(
        ...
        from om.server.billing.service import _make_billing_request
...
class TestCreateCheckoutSession:
    ...
    @pytest.mark.asyncio
    @patch("om.server.billing.service._make_billing_request")
    async def test_creates_checkout_session(
        self,
        mock_request: AsyncMock,
    ) -> None:
        """Should create checkout session and return URL."""
        from om.server.billing.service import create_checkout_session
...
class TestCreateCustomerPortalSession:
    ...
    async def test_creates_portal_session(
        ...
        from om.server.billing.service import create_customer_portal_session
...
class TestGetBillingInformation:
    ...
    async def test_returns_billing_info(
        ...
        from om.server.billing.service import get_billing_information
...
class TestUpdateSeatCount:
    ...
    async def test_updates_seats(
        ...
        from om.server.billing.service import update_seat_count
    ...
    async def test_includes_tenant_id_for_cloud(
        ...
        with patch("om.server.billing.service.MULTI_TENANT", True):
