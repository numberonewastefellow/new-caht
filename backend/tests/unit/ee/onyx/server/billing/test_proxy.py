...
class TestProxySeatUpdate:
    ...
    @pytest.mark.asyncio
    @patch("om.server.tenants.proxy.forward_to_control_plane")
    async def test_proxies_seat_update(
        ...
        from om.server.tenants.proxy import proxy_seat_update
...
class TestForwardToControlPlane:
    ...
    @pytest.mark.asyncio
    @patch("om.server.tenants.proxy.generate_data_plane_token")
    @patch("om.server.tenants.proxy.CONTROL_PLANE_API_BASE_URL", "https://cp.test")
...
class TestVerifyLicenseAuth:
    ...
    @pytest.mark.asyncio
    @patch("om.server.tenants.proxy.LICENSE_ENFORCEMENT_ENABLED", True)
    @patch("om.server.tenants.proxy.verify_license_signature")
    @patch("om.server.tenants.proxy.is_license_valid")
