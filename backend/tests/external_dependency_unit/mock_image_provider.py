...
def use_mock_image_generation_provider() -> (
    ...
    with (
        ...
        patch(
            "om.tools.tool_implementations.images.image_generation_tool.get_image_generation_provider",
            return_value=image_gen_provider,
        ),
        # Mock is_available to return True so the tool is registered
        patch(
            "om.tools.tool_implementations.images.image_generation_tool.ImageGenerationTool.is_available",
            return_value=True,
        ),
        # Mock the config lookup in tool_constructor to return a valid LLMConfig
        patch(
            "om.tools.tool_constructor._get_image_generation_config",
