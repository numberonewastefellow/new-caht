"""drop_opensearch_migration_tables

Drops the Vespa->OpenSearch migration bookkeeping tables. These backed the
migration task family and the runtime retrieval toggle, all of which were
removed when Vespa was deleted and OpenSearch became the only document index.
The tables are dead (nothing reads or writes them anymore).

`upgrade` here is the exact reverse of cbc03e08d0f3_add_opensearch_migration_tables
(it mirrors that revision's `downgrade`); `downgrade` recreates the tables (mirrors
that revision's `upgrade`) so the change stays reversible.

Revision ID: b7d4e91a2c83
...
def downgrade() -> None:
    # 1. Recreate opensearch_document_migration_record table.
    op.create_table(
    ...
    # 3. Recreate opensearch_tenant_migration_record table (singleton).
    op.create_table(
    ...
    # 4. Recreate unique index on constant to enforce singleton pattern.
    op.execute(
        sa.text(
            """
