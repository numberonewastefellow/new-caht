...
def get_model_app() -> FastAPI:
    application = FastAPI(
        title="VertualAI Model Server", version=__version__, lifespan=lifespan
    )
    if SENTRY_DSN:
        sentry_sdk.init(
            dsn=SENTRY_DSN,
...
if __name__ == "__main__":
    logger.notice(
        f"Starting VertualAI Model Server on http://{MODEL_SERVER_ALLOWED_HOST}:{str(MODEL_SERVER_PORT)}/"
    )
