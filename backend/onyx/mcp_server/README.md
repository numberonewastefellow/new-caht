# VertualAI MCP Server

## Overview

The VertualAI MCP server allows LLMs to connect to your VertualAI instance and access its knowledge base and search capabilities through the [Model Context Protocol (MCP)](https://modelcontextprotocol.io/).

With the VertualAI MCP Server, you can search your knowledgebase,
give your LLMs web search, and upload and manage documents in VertualAI.

All access controls are managed within the main VertualAI application.

### Authentication

Provide an VertualAI Personal Access Token or API Key in the `Authorization` header as a Bearer token.
...
The MCP server is built on [FastMCP](https://github.com/jlowin/fastmcp) and runs alongside the main VertualAI API server:
...
Search the user's private knowledge base indexed in VertualAI. Returns ranked documents with content snippets, scores, and metadata.
...
The MCP Server automatically launches with the `Run All VertualAI Services` task from the default launch.json.
...
4. Paste your VertualAI bearer token
5. Click **Connect**
...
- `API_SERVER_URL_OVERRIDE_FOR_HTTP_REQUESTS`: Optional override URL. If set, takes precedence over the protocol/host variables. Used for self-hosting the MCP server with VertualAI Cloud as the backend.