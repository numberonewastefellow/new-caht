...
func TestLitellmSingletonEagerImportDetection(t *testing.T) {
	...
	testContent := `
import os
from om.llm.litellm_singleton import litellm  # Should be flagged as eager import
from typing import Dict
...
`
	...
	patterns := createPatterns([]string{"om.llm.litellm_singleton"})
	result := findEagerImports(testPath, patterns)
	...
	if _, ok := result.ViolatedModules["om.llm.litellm_singleton"]; !ok {
		t.Error("Expected om.llm.litellm_singleton in violated modules")
	}
...
func TestDefaultLazyImportModules(t *testing.T) {
	...
	expectedLitellmIgnores := []string{
		"om/llm/litellm_singleton/__init__.py",
		"om/llm/litellm_singleton/config.py",
		"om/llm/litellm_singleton/monkey_patches.py",
	}
...
func TestIsValidPythonFile(t *testing.T) {
	tests := []struct {
		...
		{"regular python file", "om/main.py", true},
		{"non-python file", "om/main.go", false},
		{"test file prefix", "om/test_main.py", false},
		{"test file suffix", "om/main_test.py", false},
		...
		{"regular in subdir", "om/connectors/file.py", true},
	}
...
func TestShouldCheckFileForModule(t *testing.T) {
	...
	// Create test file
	testFile := filepath.Join(tmpDir, "om", "llm", "test.py")
	...
	settings = NewLazyImportSettings("om/llm/test.py")
	if shouldCheckFileForModule(testFile, tmpDir, settings) {
