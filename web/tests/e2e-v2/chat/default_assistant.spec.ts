...
import { loginAsRandomUser, loginAs } from "@tests/e2e-v2/utils/auth";
import {
  sendMessage,
  startNewChat,
  verifyAssistantIsChosen,
  verifyDefaultAssistantIsChosen,
} from "@tests/e2e-v2/utils/chatActions";
import {
  TOOL_IDS,
  openActionManagement,
  waitForUnifiedGreeting,
} from "@tests/e2e-v2/utils/tools";
...
test.describe("Default Assistant Tests", () => {
  ...
  test.beforeAll(async ({ browser }) => {
    ...
    const apiClient = new OmApiClient(adminPage.request);
    try {
