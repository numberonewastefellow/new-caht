...
import { loginAs, loginAsWorkerUser, apiLogin } from "@tests/e2e-v2/utils/auth";
import { OmApiClient } from "@tests/e2e-v2/utils/omApiClient";
import {
  startMcpOauthServer,
  McpServerProcess,
} from "@tests/e2e-v2/utils/mcpServer";
import { TEST_ADMIN_CREDENTIALS } from "@tests/e2e-v2/constants";
import { logPageState } from "@tests/e2e-v2/utils/pageStateLogger";
import {
  getPacketObjectsByType,
  sendMessageAndCaptureStreamPackets,
} from "@tests/e2e-v2/utils/chatStream";
...
test.describe("MCP OAuth flows", () => {
  ...
  test.beforeAll(async ({ browser }, workerInfo) => {
    ...
    const adminClient = new OmApiClient(adminPage.request);
    try {
  ...
  test("Admin can configure OAuth MCP server and use tools end-to-end", async ({
    ...
    const adminApiClient = new OmApiClient(page.request);
  ...
  test("Curator flow with access isolation", async ({
    ...
    const curatorApiClient = new OmApiClient(page.request);
