...
import { loginAs, loginAsRandomUser } from "@tests/e2e-v2/utils/auth";
...
test.describe("Assistant Creation and Edit Verification", () => {
  ...
  test.describe("User Files Only", () => {
    ...
    test.afterAll(async ({ browser }: { browser: Browser }) => {
      if (userFilesAssistantId !== null) {
        ...
        const cleanupClient = new OmApiClient(page.request);
  ...
  test.describe("With Knowledge", () => {
    ...
    test("should create and edit assistant with Knowledge enabled", async ({
      ...
      const omApiClient = new OmApiClient(page.request);
      ccPairId = await omApiClient.createFileConnector("Test Connector");
      documentSetId = await omApiClient.createDocumentSet(
        "Test Document Set",
        [ccPairId]
      );
