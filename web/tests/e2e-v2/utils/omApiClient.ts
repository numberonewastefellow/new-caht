...
/**
 * API Client for VertualAI (om) backend operations in E2E tests.
 *
 * Provides a type-safe, abstracted interface for interacting with the om backend API.
 ...
 *
 * **Chat Sessions:**
 * - `createChatSession(description, agentId?)` - Creates a chat session with a description
 * - `deleteChatSession(chatId)` - Deletes a chat session
 *
 * **Workspaces:**
 * - `createWorkspace(name)` - Creates a workspace with a name
 * - `deleteWorkspace(workspaceId)` - Deletes a workspace
 *
 * **Usage Example:**
 * ```typescript
 * // From a test with a Page:
 * const client = new OmApiClient(page.request);
 *
 ...
 * const client = new OmApiClient(ctx, baseURL);
 * ```
 *
...
export class OmApiClient {
  ...
  private async handleResponseSoft(
    ...
    if (!response.ok()) {
      const errorText = await response.text();
      console.error(
        `[OmApiClient] ${errorMessage}: ${response.status()} - ${errorText}`
      );
      return false;
    }
    return true;
   ...
   *
   * @param message - The message to log (will be prefixed with "[OmApiClient]")
   */
  private log(message: string): void {
    console.log(`[OmApiClient] ${message}`);
  }
  ...
  async deleteAssistant(assistantId: number): Promise<boolean> {
    const response = await this.request.delete(
      `${this.baseUrl}/agent/${assistantId}`
    );
    const success = await this.handleResponseSoft(
      response,
  ...
  async getAssistant(assistantId: number): Promise<{
    ...
    const response = await this.get(`/agent/${assistantId}`);
  ...
  async updateDiscordGuild(
    guildId: number,
    updates: { enabled?: boolean; default_agent_id?: number | null }
  ): Promise<{
    id: number;
    guild_id: string | null;
    guild_name: string | null;
   ...
   * @param agentId - The agent/assistant ID to use (defaults to 0)
  ...
  /**
   * Creates a workspace with a specific name.
   *
  ...
  async createWorkspace(name: string): Promise<number> {
    ...
    this.log(`Created workspace: ${name} (ID: ${data.id})`);
    return data.id;
  }
  ...
  async deleteWorkspace(workspaceId: number): Promise<void> {
    const response = await this.delete(`/workspaces/${workspaceId}`);
    await this.handleResponseSoft(
      response,
